...
export default function ConfirmEstadoDialog({ pending, bulkCount, onConfirm, onCancel }) {
  const cfg = ESTADOS[pending.nuevoEstado];
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 350, background: "rgba(27,28,57,0.45)", display: "flex", alignItems: "center", justifyContent: "center" }}
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: "#4F6867", margin: "0 0 20px" }}>
          {pending.isBulk
            ? <>{pending.pedido ? "" : `${bulkCount} pedido${bulkCount > 1 ? "s" : ""}`} → <strong style={{ color: cfg?.color }}>{cfg?.label || pending.nuevoEstado}</strong></>
            : <>{pending.pedido?.cliente || pending.pedido?.titulo || "Pedido"} → <strong style={{ color: cfg?.color }}>{cfg?.label || pending.nuevoEstado}</strong></>
          }
        </p>
